...
-- SELECT * FROM [dbo].[extended_events_file]
GO
----------------------------------------------------------------------------------------
-- open the files
...
--CREATE TABLE #event_data (event_id int identity(1,1) primary key, event_data xml NULL)

DECLARE f CURSOR 
FOR 
SELECT TOP 1 [extended_events_file_id], [file_path_and_name] 
FROM [dbo].[extended_events_file] as f
ORDER BY [extended_events_file_id]
--WHERE NOT EXISTS (
--	SELECT TOP 1 1
--	FROM dbo.extended_event as e
--	WHERE f.extended_events_file_id = e.extended_events_file_id
--)
OPEN f
...
BEGIN
	...
	SELECT @fetchStatus = @@FETCH_STATUS, @fetchStatus_nested = 0

	IF @fetchStatus = 0
	BEGIN
		PRINT @extended_events_file_path_and_name
		...
		INSERT #event_data ([event_data])
		SELECT ee.event_data 
		FROM ee

		DECLARE e_events CURSOR 
		FOR
		SELECT TOP 10 event_data FROM #event_data ORDER BY event_id;

		OPEN e_events

		BEGIN DIALOG @dialog_handle
		FROM SERVICE commandQueueInitiatorService
		TO SERVICE 'commandExtendedEventTargetService' 
		ON CONTRACT [//ExtendedEventsContract]
		WITH ENCRYPTION = OFF;

		WHILE @fetchStatus_nested = 0
		BEGIN
			FETCH NEXT FROM e_events INTO @event_data;

			SELECT @fetchStatus_nested = @@FETCH_STATUS;

			IF @fetchStatus_nested = 0
			BEGIN
...
BEGIN
	...
	WHERE ee.event_id BETWEEN 2060000 AND 2070000
	AND NOT EXISTS (
		SELECT TOP 1 1
		FROM dbo.extended_event_values as ev
	...
	UPDATE [dbo].[extended_event]
	SET extended_event = NULL
...
select top 1000 * from dbo.extended_event


select top 1000 * from dbo.extended_event_values
...
BEGIN
	...
	BEGIN
		BEGIN TRANSACTION
			...
			SEND ON CONVERSATION @dialog_handle
			MESSAGE TYPE [//ExtendedEventMessageTypeInitiator] (CAST(@event_data as nvarchar(max)));
		COMMIT TRANSACTION
	END
END
